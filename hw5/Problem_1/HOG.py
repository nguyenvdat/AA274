...
def tf_histogram_of_oriented_gradients(img_raw, x_kernel=[-1.0, 0.0, 1.0], y_kernel=[-1.0, 0.0, 1.0], pixels_in_cell=8, cells_in_block=1, n_angle_bins=9):
    ...
    img = tf.cast(img_raw, tf.float32)                                                                             # convert int pixel values to float
    ...
    grad_ang = tf.math.mod(grad_ang, np.pi)    # unsigned gradients
    ...
    hi_bin = tf.math.mod(hi_bin, n_angle_bins)

    lo_bin = tf.cast(lo_bin, tf.int32)
    hi_bin = tf.cast(hi_bin, tf.int32)
