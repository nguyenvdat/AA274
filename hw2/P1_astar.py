...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        return (x[0] >= self.statespace_lo[0]) and (x[0] <= self.statespace_hi[0]) and (x[1] >= self.statespace_lo[1]) and (x[1] <= self.statespace_hi[1]) and self.occupancy.is_free(x)
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
                if i != 0 or j != 0:
                    neigh = self.snap_to_grid(
                        (x[0] + i*self.resolution, x[1] + j*self.resolution))
    ...
    def solve(self):
        ...
        self.est_cost_through[self.x_init] = self.distance(
            self.x_init, self.x_goal)
        while len(self.open_set) > 0:
            x_current = self.find_best_est_cost_through()
            if x_current == self.snap_to_grid(self.x_goal):
                self.came_from[self.x_goal] = self.came_from[x_current]
            ...
            for x_neigh in self.get_neighbors(x_current):
                if x_neigh in self.closed_set:
                    continue
                tentative_cost_to_arrive = self.cost_to_arrive[
                    x_current] + self.distance(x_current, x_neigh)
